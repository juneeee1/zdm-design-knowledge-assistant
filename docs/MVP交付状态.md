...
- 发布链接、最终测试结果将在远程验证后更新；不把创建项目当作部署成功。

## 验证边界

完整性校验、检索/API/安装器与私有存储并发测试由 `npm run validate` 和 `npm test` 运行。模型适配器使用本地 HTTP 测试服务验证请求与返回格式，不是线上真实模型验收。

完整 tgz 已通过 npm exec 安装到全新临时项目，安装后的规范查询和双卡查询成功。宿主 AI 的自动发现/触发体验仍需在实际新会话中验收。

新 HTTP 应用已在 Chrome 打开并进行桌面交互与截图检查；双卡查询、原图、卡片筛选与对照可用。移动端、反馈持久化与公网资源将在发布后继续核验。
