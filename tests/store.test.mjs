...
test("private blob store retries conflicts without losing feedback", async () => {
  ...
  const reserved = await Promise.all([
    store.reserveAI("2026-09-14", 1),
    store.reserveAI("2026-09-14", 1),
  ]);
  assert.equal(reserved.filter(Boolean).length, 1);
  assert.equal(JSON.parse(text).totalAI, 1);
});
test("blob store limits feedback and fails closed when storage is unavailable", async () => {
  const state = {
    ...
    used: 0,
    totalAI: 0,
    feedback: Array.from({ length: 200 }, () => ({ id: "old" })),
  };
